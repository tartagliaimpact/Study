...
namespace лаб3_ООП
{
    ...
    public partial class Form1 : Form
    {
        ...
        IndexAttribute Indx = new IndexAttribute();
        ...
        private void DataGridRowAdd(DataGridView dgv)
        {
            try
            {
                string ix = indexbox.Text;
            if (Indx.IsValid(ix) == false)
            {
                throw new Exception(Indx.ErrorMessage);
            }
                ...
                this.comboBox1.SelectedItem, this.comboBox2.SelectedItem, this.floorNuD.Value, this.indexbox.Text,
        ...
        private void AddAp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                apartments.Add(new Apartment(new Adress(this.comboBox3, this.comboBox4, this.comboBox5, this.comboBox6, this.comboBox7, this.hScrollBar2, this.trackBar1, this.indexbox),
                    this.footgtb, this.colkomnud, this.checkBox1, this.checkBox2, this.checkBox3, this.checkBox4, this.checkBox5,
                    this.comboBox1, this.comboBox2, this.floorNuD, this.CostBox));
                ...
                Apartment.Clear(this.comboBox3, this.comboBox4, this.comboBox5, this.comboBox6, this.comboBox7, this.hScrollBar2, this.trackBar1,
                    this.footgtb, this.colkomnud, this.checkBox1, this.checkBox2, this.checkBox3, this.checkBox4, this.checkBox5,
                    this.comboBox1, this.comboBox2, this.floorNuD, this.indexbox, this.CostBox);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        //-----------------------------------------------------------------
        //Счётчики для trackBar и hscrollBar
        //-----------------------------------------------------------------
        ...
        {
            MessageBox.Show("Версия 3.0;  Разработчик: Карпин А.В.");
        }

        //-----------------------------------------------------------------
        //Сортироовка по площади
        //-----------------------------------------------------------------

        private void toolStripMenuItem7_Click(object sender, EventArgs e)
        {
            dataGridView1.Rows.Clear();
            var orderedbyfootage = from i in apartments
                                   orderby i.footage
                                   select i;
            foreach (Apartment a in orderedbyfootage)
            {
                dataGridView1.Rows.Add(a.footage, a.roomcolich, a.kitchen, a.bath, a.toilet, a.basement, a.balcony,
                a.year_of_building, a.material_type, a.floor, a.adress.index,
                "" + a.adress.country + ", город " + a.adress.city + ", район : "
        ...
        private void поУбываниюToolStripMenuItem_Click(object sender, EventArgs e)
        {
            dataGridView1.Rows.Clear();
            var orderedbyfootage = from i in apartments
                                   orderby i.roomcolich descending
                                   select i;
            foreach (Apartment a in orderedbyfootage)
            {
                dataGridView1.Rows.Add(a.footage, a.roomcolich, a.kitchen, a.bath, a.toilet, a.basement, a.balcony,
        ...
        //-----------------------------------------------------------------
        //Сохранение и чтение результатов сортировки
        //-----------------------------------------------------------------
        ...
        private void сохранитьРезультатыСортировкиToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                XmlSerializer xml = new XmlSerializer(typeof(List<Apartment>));
                using (FileStream fs = new FileStream("ApartmentsSorted.xml", FileMode.OpenOrCreate))
                {
                    xml.Serialize(fs, apartments);
                }
                MessageBox.Show("Данные уcпешно сохранены на xml");
        ...
        private void прочитатьИзФайлаToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                XmlSerializer xml = new XmlSerializer(typeof(List<Apartment>));
                using (FileStream fs = File.OpenRead("ApartmentsSorted.xml"))
                {
                    List<Apartment>? bufflist = new List<Apartment>();
                    bufflist = xml.Deserialize(fs) as List<Apartment>;
                    foreach (Apartment a in bufflist)
        ...
        {
            SearchForm searchForm = new SearchForm(apartments);
            searchForm.Show();
        }
        ...
        //-----------------------------------------------------------------
        //Очистка на панели инструментов
        //-----------------------------------------------------------------
